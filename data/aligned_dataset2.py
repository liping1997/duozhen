...
class AlignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.dir_ABB = os.path.join(opt.dataroot, opt.phase)  # get the image directory
        self.ABB_paths = sorted(make_dataset(self.dir_ABB, opt.max_dataset_size))  # get image paths
    ...
    def __getitem__(self, index):
        ...
        ABB_path = self.ABB_paths[index]
        ABB = Image.open(ABB_path).convert('RGB')
        ...
        return {'A': A, 'B0': B0, 'B1': B1, 'A_paths': ABB_path, 'B0_paths': ABB_path, 
                'B1_paths': ABB_path}

    def __len__(self):
        """Return the total number of images in the dataset."""
        return len(self.ABB_paths)
